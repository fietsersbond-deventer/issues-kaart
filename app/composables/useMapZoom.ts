...
export function useMapZoom(
  ...
  function zoomToAllIssues(allIssues: Issue[]) {
    if (!mapRef.value?.map || allIssues.length === 0) {
      console.debug('[useMapZoom] skipping zoomToAllIssues - no map or no issues');
      return;
    }

    console.debug('[useMapZoom] zooming to all issues', { count: allIssues.length });
    ...
    (issues) => {
      ...
      if (!hasInitialZoom.value && issues.length > 0) {
        ...
        if (selectedIssue.value?.geometry) {
          hasInitialZoom.value = true;
          console.debug('[useMapZoom] skipping zoom to all - selected issue already loaded');
          return;
        }
        ...
        initialZoomTimer = setTimeout(() => {
          ...
          if (!selectedIssue.value?.geometry) {
            ...
            hasInitialZoom.value = true;
            console.debug('[useMapZoom] cancelled - selected issue loaded in the meantime');
          }
          initialZoomTimer = null;
        }, 100); // 100ms delay to wait for selectedIssue
        
        console.debug('[useMapZoom] scheduled delayed zoom to all issues (100ms)');
      }
    },
    { immediate: true }
  );
  ...
  watch(
    ...
    (newIssue, oldIssue) => {
      ...
      if (newIssue?.geometry) {
        ...
        if (initialZoomTimer) {
          console.debug('[useMapZoom] cancelling zoom to all - selected issue arrived');
          clearTimeout(initialZoomTimer);
          initialZoomTimer = null;
        }
        ...
        if (!oldIssue || oldIssue?.id !== newIssue.id) {
          ...
          console.debug('[useMapZoom] reset hasInitialSelectedZoom for new/different issue');
        }
  ...
  watch(
    ...
    ([width, height]) => {
      ...
      if (width > 0 && height > 0 && selectedIssue.value?.geometry && mapRef.value?.map) {
        // Reset flag since we're about to do a fresh zoom
        hasInitialSelectedZoom.value = false;
        console.debug('[useMapZoom] reset hasInitialSelectedZoom for mapSize-triggered zoom');
        zoomToSelectedIssue();
      }
    }
  );
  ...
  watch(
    ...
    (newPadding, oldPadding) => {
      ...
      const hasChanged = newPadding.some((val, idx) => val !== oldPadding?.[idx]);
      ...
      if (hasChanged && !hasInitialSelectedZoom.value && selectedIssue.value?.geometry) {
        console.debug('[useMapZoom] first padding update - marking initial zoom complete');
        hasInitialSelectedZoom.value = true;
        return;
      }
      
      // Subsequent padding changes - re-zoom (e.g., legend opened/closed)
      if (hasChanged && hasInitialSelectedZoom.value && selectedIssue.value?.geometry && mapRef.value?.map) {
